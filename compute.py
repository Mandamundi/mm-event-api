...
def fetch_price_series(ticker: str, start: str, end: str):
    ...
    # Stooq uses different symbols for indices and futures
    STOOQ_MAP = {
        "^GSPC":    "^SPX",
        "^NDX":     "^NDX",
        "^DJI":     "^DJI",
...
def compute_aggregate_stats(event_results: list, pre_days: int, post_days: int) -> dict:
    ...
    return {
        "t_axis": t_axis,
        "median": [round(v, 4) if not math.isnan(v) else None for v in median_line],
        "mean":   [round(v, 4) if not math.isnan(v) else None for v in mean_line],
        "p25":    [round(v, 4) if not math.isnan(v) else None for v in p25_line],
        "p75":    [round(v, 4) if not math.isnan(v) else None for v in p75_line],
        "win_rates": {
            "1w":  win_rate(5),
            "1m":  win_rate(21),
            "3m":  win_rate(63),
